...
router.post("/my-board", async(req, res) => {
    const username = req.session.username;
    boardModel.addByUser(username, req.body.new_board)
        .then(response => {
...
router.post("/delete-board", async(req, res) => {
    ...
    if (owner.owner === username) {
        const total = await boardModel.delete(req.body.boardID)
            .then(response => {
                res.json({
                    code: 0,
                    result: {
...
router.post("/edit-board", async(req, res) => {
    ...
    if (owner.owner === username) {
        ...
        const total = await boardModel.edit(req.body.edit_board)
            .then(async(response) => {
